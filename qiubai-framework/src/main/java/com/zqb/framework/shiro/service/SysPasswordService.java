...
import com.zqb.framework.web.exection.user.UserPasswordNotMatchException;
import com.zqb.framework.web.exection.user.UserPasswordRetryLimitExceedException;
...
public class SysPasswordService {
    ...
    public void validate(SysUser user, String password) {
        ...
        if (retryCount.incrementAndGet() > maxRetryCount) {
            RecordUtils.LoginInfo(loginName, Constants.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.exceed", maxRetryCount));
            throw new UserPasswordRetryLimitExceedException(maxRetryCount);
        }

        if (!matches(user, password)) {
            RecordUtils.LoginInfo(loginName, Constants.LOGIN_FAIL, MessageUtils.message("user.password.retry.limit.count", retryCount));
            loginRecordCache.put(loginName, retryCount);
